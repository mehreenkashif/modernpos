...
function date_range_filter_logs($from, $to)
{
	$from = $from ? $from : date('Y-m-d');
	$to = $to ? $to : date('Y-m-d');
	$where_query = '';
	if (($from && ($to == false)) || ($from == $to)) {
		...
		$month = date('m', strtotime($from));
		$year = date('Y', strtotime($from));
		$where_query .= " AND DAY(`login_logs`.`created_at`) = {$day}";
		$where_query .= " AND MONTH(`login_logs`.`created_at`) = {$month}";
		$where_query .= " AND YEAR(`login_logs`.`created_at`) = {$year}";
	} else {
		$from = date('Y-m-d H:i:s', strtotime($from.' '. '00:00:00')); 
		$to = date('Y-m-d H:i:s', strtotime($to.' '. '23:59:59'));
		$where_query .= " AND login_logs.created_at >= '{$from}' AND login_logs.created_at <= '{$to}'";
	}
	return $where_query;
}
...
{
	function mb_substr(...$arg)
	{
		return utf8_substr(...$arg);
	}
}

if (!function_exists('mb_strlen'))
{
	function mb_strlen(...$arg)
	{
		return utf8_strlen(...$arg);
	}
}