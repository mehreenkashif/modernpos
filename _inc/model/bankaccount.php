...
class ModelBankAccount extends Model 
{
	...
	{		
    	$statement = $this->db->prepare("INSERT INTO `bank_accounts` (account_name, account_details, account_no, contact_person, phone_number, url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
    	$statement->execute(array($data['account_name'], $data['account_details'],  $data['account_no'],$data['contact_person'], $data['phone_number'], $data['url'], date('Y-m-d H:i:s')));
