...
class ModelInvoice extends Model
{
    ...
    public function createInvoice($request, $store_id = null)
    {
        ...
        $created_at = isset($request->post['created-at']) && $request->post['created-at'] ? date('Y-m-d H:i:s', strtotime($request->post['created-at'] . ' ' . date('H:i:s'))) : date_time();
        ...
        foreach ($product_items as $product) 
        {
            ...
            while ($quantity_exist > 0) {
                ...
                if (INSTALLMENT && $request->post['is_installment_order'] && $due > 0) {
                    $statement = $this->db->prepare("UPDATE `purchase_item` SET `installment_quantity` = `installment_quantity` + {$sell_quantity} WHERE `store_id` = ? AND `item_id` = ? AND `status` = ?");
                    $statement->execute(array($store_id, $product_id, 'active'));
                }
            ...
            $statement = $this->db->prepare("INSERT INTO `selling_item` (invoice_id, store_id, item_id, category_id, brand_id, sup_id, item_name, item_purchase_price, item_price, item_discount, item_tax, tax_method, taxrate_id, tax, gst, cgst, sgst, igst, item_quantity, item_total, purchase_invoice_id, installment_quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($invoice_id, $store_id, $product_id, $category_id, $brand_id, $sup_id, $product_name, $item_purchase_price, $product_price, $product_discount, $tax, $tax_method, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total, $purchase_invoice_id, $installment_quantity));
        ...
        if (INSTALLMENT && $request->post['is_installment_order']) {
            $statement = $this->db->prepare("INSERT INTO `installment_orders` (store_id, invoice_id, purchase_invoice_id, duration, interval_count, installment_count, interest_percentage, interest_amount, initial_amount, last_installment_date, installment_end_date, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
            $statement->execute(array($store_id, $invoice_id, $purchase_invoice_id, $installment_duration, $installment_interval_count, $installment_count, $installment_interest_percentage, $installment_interest_amount, $installment_initial_amount, $created_at, $installmentEndDate, $created_at));
